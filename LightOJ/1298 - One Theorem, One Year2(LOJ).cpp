...
long long func(long long k, long long p, long long power)
{
    if(k == 0 && p == 0)
        return 1;
    ...
    long long temp, i, ret;

    temp = ((prime[p] - 1) * precal[p][power]) % mod;
    ret = (temp * func(k - 1, p - 1, 0)) % mod;

    ret += func(k - 1, p, power + 1);
